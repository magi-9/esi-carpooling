    ...
    <div style="display:flex;justify-content:space-between;align-items:flex-start">
      <div>
        ...
        <div style="margin-top:8px;color:#555">
          Distance to pickup: <strong>{{ recommendation.distanceToOriginKm?.toFixed(1) }} km</strong>
        </div>
        <div style="color:#555">
          Distance to dropoff: <strong>{{ recommendation.distanceToDestinationKm?.toFixed(1) }} km</strong>
        </div>
      </div>
      <div style="text-align:right">
        ...
        <button @click="$emit('book', recommendation.rideId)"
                style="margin-top:12px;padding:8px 16px;background:#27ae60;color:white;border:none;cursor:pointer;border-radius:4px">
          Book &amp; Pay
        </button>
      </div>
    </div>
