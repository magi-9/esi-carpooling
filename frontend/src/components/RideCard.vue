    ...
    <div style="display: flex; justify-content: space-between; align-items: flex-start">
      <div>
        <div style="font-weight: bold; font-size: 16px">
          {{ ride?.startAddress || 'Unknown' }} → {{ ride?.endAddress || 'Unknown' }}
        </div>
        ...
        <div style="margin-top: 8px; color: #555">
          <span>📅 {{ formatDate(ride?.rideStartDate) }}</span>
          <span style="margin-left: 16px">💺 {{ ride?.availableSeats }} seats</span>
          <span style="margin-left: 16px">💰 {{ ride?.seatPriceAmount }} {{ ride?.seatPriceCurrency }}</span>
        </div>
...
const ride = computed(() => props.recommendation?.ride);
