<template>
  <div style="padding: 24px; max-width: 900px; margin: 0 auto">
    ...
    <div
      v-else
      style="border: 1px solid #27ae60; border-radius: 8px; padding: 24px; max-width: 400px"
    >
      <h3 style="color: #27ae60">Refund Submitted</h3>
      <table style="width: 100%; border-collapse: collapse">
        <tr>
          <td style="padding: 4px 0; color: #666">Refund ID:</td>
          <td style="font-size: 12px">{{ refund.refundId }}</td>
        </tr>
        <tr>
          <td style="padding: 4px 0; color: #666">Amount:</td>
          <td>{{ refund.refundedAmount?.amount }} {{ refund.refundedAmount?.currency }}</td>
        </tr>
        <tr>
          <td style="padding: 4px 0; color: #666">Reason:</td>
          <td>{{ refund.reason }}</td>
        </tr>
        <tr>
          <td style="padding: 4px 0; color: #666">Processed:</td>
          <td>{{ new Date(refund.processedAt).toLocaleString() }}</td>
        </tr>
      </table>
    </div>
  </div>
